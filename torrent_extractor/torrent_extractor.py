...
def main():
    ...
    parser.add_argument(
        "-t",
        "--tv_path",
        help="path to destination folder for TV shows, default is /tmp/films")
    parser.add_argument(
        "-f",
        "--film_path",
        help="path to destination folder for films, default is /tmp/tv_series")
    parser.add_argument(
        "-d",
        "--debug",
        action="store_true",
        help="""set this flag to prevent log files from being written and
        enable DEBUG-level output to console"""
    )
    parser.add_argument(
        "-l",
        "--log_file",
        help="path to log file, default is ./torrent_extractor.log")
    parser.add_argument(
        "torrent", help="the file path to the torrent to extract")
    ...
    # Set up a file handler that also rotates the logs, unless of course debug
    # mode is set, then we only log to console
    if not args.debug and os.access(
            os.path.dirname(os.path.abspath(log_file)), os.W_OK):
    ...
    if not os.path.exists(os.path.normpath(args.torrent)):
        log.critical(
            os.path.normpath(args.torrent) + " does not exist, exiting.")
        sys.exit(1)
    if not os.access(os.path.abspath(log_file), os.W_OK):
        log.warning("No write permission to " + log_file +
                    ", no log file will be written.")
    if not os.access(os.path.abspath(settings.tv_path), os.W_OK):
        log.critical("No write permission to " + os.path.abspath(
            settings.tv_path) + ", exiting.")
        sys.exit(1)
    if not os.access(os.path.abspath(settings.film_path), os.W_OK):
        log.critical("No write permission to " + os.path.abspath(
            settings.film_path) + ", exiting.")
        sys.exit(1)
    ...
    if not torrents:
        log.info("No torrents to unpack or extract found in " +
                 os.path.normpath(args.torrent))
